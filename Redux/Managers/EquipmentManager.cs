...
namespace Redux.Managers
{
    ...
    public class EquipmentManager
    {
        ...
        public bool EquipItem(ConquerItem item, byte location, bool updateClient = true)
        {
            ...
            if (item.EquipmentType == 1050)
            {
                ConquerItem bow;
                Owner.TryGetEquipmentByLocation(ItemLocation.WeaponR, out bow);
                if (bow == null)
                    return false;
                if (bow.EquipmentType != 500)
                    return false;

            }
            //Check all iteminfo req's           
            bool success = (item.BaseItem.GenderReq == 0 || !Owner.IsMale) &&
                (Owner.RebornCount > 0 || (Owner.Level >= item.BaseItem.LevelReq) &&
                (Owner.Strength >= item.BaseItem.StrengthReq &&
