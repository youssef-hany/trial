...
namespace Redux.Managers
{
    public class CombatManager
    {
            ...
            switch (_packet.Action)
            {
                ...
                case InteractAction.Marry:
                    ...
                    else
                    {
                        ...
                        p1.SendToScreen(StringsPacket.Create(p1.UID, StringAction.Fireworks, "[GM] Odin loves you all"), true);
                    }
                    break;

                case InteractAction.Chop:
                    var player = (Player)owner;
                    /* Tsk = new DbTask();
                     Tsk.Owner = player.UID;
                     Tsk.Count = 30;
                     Tsk.Condition = 1;
                     int Type = 1;


                     Database.ServerDatabase.Context.Tasks.UpdateTask(Tsk.Count, Tsk.Owner, Type);*
                     _packet.Killed = 20;*/
                    // var task = Database.ServerDatabase.Context.Tasks.GetTasksByPlayerUID(player.UID, 1);
                    //_packet.Killed = 0;//(uint)task.Count;
                    player.Send(_packet);
        ...
        #region ProcessJar
        private void ProcessJar(InteractPacket _packet)
        {
            var player = (Player)owner;
            ...
            else
            {
                ...
                if (owner is Player)
                {
                    ...
                    if (owner != null && skill != null) {
                        ...
                        catch (NullReferenceException p) { Console.WriteLine("Wrong with skill lvl in player {0} Error: {1}", owner.Name, p); }
        ...
        private void LaunchHealTargetSkill()
        {
            ...
            {
                var TM = owner as Player;
                var player = PlayerManager.GetUser(TM.Name);
                packet.Data = target.UID;
               
                if (!skill.Multi && owner != target) // healing target inteamornot
                {
                    packet.AddTarget(target.UID, (uint)skill.Power);
                    target.Life += (ushort)skill.Power;
                    if ((owner is Player) && target.Life != target.MaximumLife)
                    { AddSkillExperience(skill.ID, (ulong)skill.Power); }      
                }
                else if (!skill.Multi && owner == target) //healing ownself inteamornot
                {
                    packet.AddTarget(owner.UID, (uint)skill.Power);
                    owner.Life += (ushort)skill.Power;
                    if ((owner is Player) && owner.Life != owner.MaximumLife)
                    { AddSkillExperience(skill.ID, (ulong)skill.Power); }
                }

                if (skill.Multi && owner != target && player.Team == null)
            {
                    return;
            }
                ...
                else if (player != null && player.Team != null && skill.Multi)
                {
                    bool exptest = false;
                    
                    foreach (var p in player.Team.Members)
                    {
                        
                        
                        if (p == player || !p.Alive || !p.VisibleObjects.ContainsKey(player.UID))
                        { continue; }

                        else
                        {

                            if ((owner is Player) && owner == target)
                            {
                                packet.AddTarget(owner.UID, (uint)skill.Power);
                                packet.AddTarget(p.UID, (uint)skill.Power);
                                p.Life += (ushort)skill.Power;
                                owner.Life += (ushort)skill.Power;
                                if (exptest == false && (p.Life != p.MaximumLife || owner.Life != owner.MaximumLife))
                                {
                                    exptest = (p.Life != p.MaximumLife || owner.Life != owner.MaximumLife);
                                    AddSkillExperience(skill.ID, (ulong)skill.Power);
                                }
                            }
                            else
        ...
        private void LaunchSectorSkill()
        {
            ...
            if (owner is Player && skill != null && expGain > 0)
            {
                try
                {
                    ((Player)owner).GainExperience(expGain);
                    AddSkillExperience(skill.ID, expGain);
                }catch(NullReferenceException p) { Console.WriteLine("Player {0}, problem with skill exp Error:{1}", owner.Name, p); }
            }
        }
        #endregion
        ...
        private void AutoEquip(ConquerItem item, uint id)
        {
            ...
            bool EqAr = owner.Equipment.TryGetItemBySlot(arloc, out item);

            ConquerItem RemovedArrow = null;

          
                _owner.DeleteItem(item, true);
            ...
            if (hasitem == true)
            {
                EqAr = owner.Equipment.TryGetItemBySlot(arloc, out item);
                if (EqAr)
                {
            ...
            else
            {
                ...
                if (!hasitem)
                {
                    ...
                    for (int i = 0; i < 3; i++)
                    {
                        ...
                        {
                            EqAr = owner.Equipment.TryGetItemBySlot(arloc, out item);
                            RemovedArrow = _owner.GetItemByID((uint)id);
                            if (EqAr || RemovedArrow.Durability <= 2)
                            {
                                owner.Equipment.UnequipItem(5);
                                _owner.DeleteItem(item, true);
                            }
            ...
            if (_owner.ArrowAutoEq(id, arloc))
            {
            ...
            else
            {
                ...
                EqAr = owner.Equipment.TryGetItemBySlot(5, out item);
                if (EqAr && !_owner.ArrowAutoEq(id, arloc))
                {
                    owner.Equipment.UnequipItem(5);
                    _owner.DeleteItem(item, true);
