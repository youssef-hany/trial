...
namespace Redux.Game_Server
{
    public abstract class Entity : ILocatableObject
    {
        ...
        public virtual int AttackRange { get { return CombatStats.AttackRange; } }
        ...
        {
            if (_target is Player || _target is SOB || _target is Pet) return 0;
        ...
        public virtual uint CalculatePhysicalDamage(Entity _target, DbMagicType _spell, bool _canDodge = false)
        {
            ...
            var dmg = Common.Random.Next(CombatStats.MinimumDamage, CombatStats.MaximumDamage) * (CombatStats.BonusAttackPct + CombatStats.DragonGemPct) / 230;
            if (_target is Monster)
            {
                dmg *= GetLevelBonusDamageFactor(Level, _target.Level);
            ...
            dmg -= ( _target.CombatStats.BlessPct) ;
            dmg -= ( _target.CombatStats.TortGemPct);
