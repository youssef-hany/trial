        ...
        public static void Process_GeneralActionPacket(Player client, GeneralActionPacket packet)
        {
            switch (packet.Action)
            {
                ...
                case DataAction.Hotkeys:
                    ...
                    PlayerManager.SendToServer(new TalkPacket(ChatType.Task, client.Name + " Has logged onto the server!"));
                    if (client.Map.MapInfo.ID == 3024)
                    {
                        client.ChangeMap(1002);
                    }
                    client.SendMessage("Welcome to Requiem!", ChatType.GM, ChatColour.Green);
                    //client.SendMessage("", ChatType.Talk2);
                    //client.SendMessage("", ChatType.Talk2);
                    if (client.Character.Newbie == 0)
                    {
                        client.SendMessage("WELCOME TO REQUIEM ONLINE! do not hesitate to contact any GM at any time if you have questions.");
                       //client.SendMessage("", ChatType.Talk2);
                        //client.SendMessage("", ChatType.Talk2);
                        client.Newbiequest = 1;
                        //client.SendMessage("Now , you must go to complete ( Newbie Quest ) , Kill 10 Pheasant!");
                    }
                    client.Send(packet);
                    client.Send(packet);
                    break;
        ...
        public static void Process_TeamInteractPacket(Player client, TeamInteractPacket packet)
        {
            ...
            #region Disconnected
                ...
                if (!client.Character.Online)
                {
                    ...
                    client.Team.SendToTeam(new TalkPacket(ChatType.System, client.Name + " has Disconnected."));
                    client.Team = null;
                }
