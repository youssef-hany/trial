...
namespace Redux
{
    public static class Commands
    {
        ...
        private static void Process_Level(Player client, string[] command)
        {
            ...
            { client.SetLevel(val); client.Experience = 0; client.Save(); }
            else
        ...
        private static void Process_Map(Player client, string[] command)
        {
            ...
            { client.ChangeMap(id, x, y); client.Save(); }
            else
        ...
        private static void Process_Test(Player client, string[] command)
        {
            ...
            try
            {
                ...
                switch (command[1].ToLower())
                {
                    ...
                    case "fire":
                        client.CombatManager.AddOrUpdateSkill(SkillID.Thunder, 4);
                        client.CombatManager.AddOrUpdateSkill(SkillID.Fire, 3);
                        client.CombatManager.AddOrUpdateSkill(SkillID.Cure, 4);
                        client.CombatManager.AddOrUpdateSkill(SkillID.Tornado, 3);
        ...
        private static void Process_Reborn(Player client, string[] command)
        {
            ...
            {
                client.SendMessage(" 2nd rb", ChatType.System);
                using (var session = NHibernateHelper.OpenSession())
                {
                    var t = session.CreateSQLQuery("UPDATE Characters SET Profession1=15  WHERE UID=" + client.UID);
                    t.ExecuteUpdate();
